...
# router defintion for enabling separate files outside of main.py
router = APIRouter(
    prefix='/posts',
    tags=['Posts']
)

# TODO: 'get_posts' works as advertised. May 8th, 2023 - 2233
...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    posts = (db.query(models.Post, func.count(models.Votes.post_id)
                .label("votes"))
                .join(models.Votes, 
                    models.Votes.post_id == models.Post.id, isouter=True)
                .group_by(models.Post.id)
                .filter(search_term).limit(limit).offset(skip).all())
